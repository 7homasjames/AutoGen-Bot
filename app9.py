...
def get_response(user_input, file_type):
    ...
    try:
        ...
        autogen_output = result
        #print("Autogen Output:", autogen_output)
...
if option == "🔍 SQL Query Generation":
    ...
    st.title("Multi Agent RAG Question Answering 💬")
    ...
    if user_input:
        ...
        chat_history = response["output"]["chat_history"]
        #print("Chat History:", chat_history)

        # Extract the latest non-empty assistant response
        final_response = response["output"]["chat_history"][1]["content"]
        # Print the extracted response
        #print("Extracted Response:", next(message["content"] for message in reversed(chat_history) ))
        print("Chat_History:",response["output"]["chat_history"][2]["content"])
    ...
    # Print dictionary format
        ...
        st.session_state.chat_history.append((final_response, True))
        #st.chat_message("assistant").write(final_response)

        chat_history = response["output"]["chat_history"]
        st.chat_message("assistant").write(chat_history[1]["content"])
        for i in range(2, len(chat_history)):  # Start from index 1
